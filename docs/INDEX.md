...
## Overview
Complete documentation for the SkillSynth adaptive learning platform — FastAPI + SQLAlchemy backend, two Next.js frontends (student ar/en RTL-first :3000, admin English-only :3001), and a strict-3NF 15-table database.
...
| 07 | [Backend](07-backend/INDEX.md) | Routers/services/repositories/entities layout |
| 08 | [Frontend](08-frontend/INDEX.md) | Student app structure, i18n, state |
| 09 | [Admin](09-admin/INDEX.md) | Admin app (:3001), CRUD pages, binary admin gate |
...
| # | Section | Description |
|---|---------|-------------|
| 22 | [API](22-api/INDEX.md) | Endpoint reference — 63 operations / 49 paths |
...
| 28 | *(removed)* | Gamification section deleted — feature removed (ADR-013) |
...
| 41 | [Decision Records](41-decision-records/INDEX.md) | ADR index incl. supersessions |
...
| 44 | [Test Scenarios](44-test-scenarios/INDEX.md) | Suite map: 142 tests / 11 files |
...
## Quick Reference
- **Run**: `python run.py` (:8000) · `cd src/frontend && pnpm dev` (:3000) · `cd src/admin-app && pnpm dev` (:3001)
- **Seed**: `PYTHONPATH=src python seed_v3.py` (~1100 rows, 15 tables)
- **Verify**: `PYTHONPATH=src python -m pytest tests/ -q` (142 passed) · `PYTHONPATH=src python tools/verify_schema.py` (SCHEMA MATCH) · `pnpm type-check && pnpm lint && pnpm build` per app
- **API**: 63 operations across 49 paths; OpenAPI at :8000/docs
- **Status**: August 2026 — integrity layer shipped, restricted deletes live (ADR-014 pending)

## File Stats
- **Sections**: 50 numbered slots (28 retired after feature removal)
