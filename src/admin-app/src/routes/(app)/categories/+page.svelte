...
<Panel title="Skill categories">
  <div class="toolbar">
    <Button variant="primary" onclick={openCreate}><Icon name="plus" size={16} /> Add category</Button>
  </div>
  {#if loading}
  ...
  {:else}
    <table class="grid-table">
      <thead>
        <tr><th>ID</th><th>Name</th><th>Description</th><th>Parent</th><th></th></tr>
      </thead>
      <tbody>
        {#each rows as r (r.id)}
          <tr>
...
<Dialog open={showForm} onclose={() => (showForm = false)} title={editing ? 'Edit category' : 'Add category'}>
  <Field label="Name">
    <Input bind:value={form.name} placeholder="Web Development" />
  </Field>
...
<Dialog open={showDelete} onclose={() => (showDelete = false)} title="Delete category">
  <p>Delete <strong>{delTarget?.name}</strong>? This cannot be undone.</p>
  {#if dependents}
    <p class="warn">This category still has related records. Force delete to remove them:</p>
    <ul class="deps">
